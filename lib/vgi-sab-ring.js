...
// Claim a free slot (CAS state 0->unique-id), reset both rings. Returns slot index
// or -1. STATE carries a UNIQUE claim id (not a constant 1) so the worker dispatcher
// can distinguish a still-active claim from a release+reclaim (see vgi_sab_abi.hpp).
function slotOpen(buf, base) {
    ...
    for (let s = 0; s < hdr.nSlots; s++) {
        ...
        if (Atomics.compareExchange(i32, sb + SLOT_STATE, SLOT_FREE, id) === SLOT_FREE) {
            i32[sb + C2W_WRITE] = 0;
            i32[sb + C2W_READ] = 0;
            i32[sb + C2W_CLOSED] = 0;
            i32[sb + W2C_WRITE] = 0;
            i32[sb + W2C_READ] = 0;
            i32[sb + W2C_CLOSED] = 0;
            i32[sb + TERMINAL_CLAIM] = 0;
            i32[sb + TERMINAL_CODE] = 0;
            i32[sb + TERMINAL_DETAIL] = 0;
            return s;
        }
    }
    return -1;
...
function stubTerminalError(buf, base, slot) {
    ...
    if (
        (Atomics.load(i32, (base >> 2) + HDR_FEATURES) & FEATURE_TERMINAL_ERROR) === 0 ||
        Atomics.load(i32, sb + SLOT_STATE) === 0 ||
        Atomics.load(i32, sb + TERMINAL_CLAIM) !== Atomics.load(i32, sb + SLOT_STATE)
    )
        return null;
    return {
        code: Atomics.load(i32, sb + TERMINAL_CODE),
        detail: Atomics.load(i32, sb + TERMINAL_DETAIL),
    };
}
...
function workerCloseW2c(buf, base, slot) {
    ...
    const sb = slotBase(base, hdr, slot);
    const i32 = new Int32Array(buf);
    Atomics.store(i32, (sb >> 2) + W2C_CLOSED, Atomics.load(i32, (sb >> 2) + SLOT_STATE));
