addToLibrary({
    ...
    vgi_wasm_slot_open: function (locationPtr, regionOffset) {
        ...
        for (var s = 0; s < hdr.nSlots; s++) {
            ...
            if (Atomics.compareExchange(i, sb + vgiSab.STATE, 0, id) === 0) {
                // Reset both rings (Atomics.store so the worker sees the zeros after the
                // notify's happens-before, before it starts serving). W2C_CLOSED is reset
                // to 0 (open); the worker will close it with THIS claim id (`id`) — a stale
                // worker for a previous claim writes the OLD id, which != `id`, so our reads
                // ignore its close (see the W2C_CLOSED == STATE check in ringRead). This is
                // what makes reclaiming a not-yet-drained slot race-free.
    ...
    vgi_wasm_slot_terminal_error: function (regionOffset, slot, codePtr, detailPtr) {
        ...
        if (
            (Atomics.load(i, (regionOffset >> 2) + vgiSab.HDR_FEATURES) & vgiSab.FEATURE_TERMINAL_ERROR) === 0 ||
            Atomics.load(i, sb + vgiSab.STATE) === 0 ||
            Atomics.load(i, sb + vgiSab.TERMINAL_CLAIM) !== Atomics.load(i, sb + vgiSab.STATE)
        )
            return 0;
        if (codePtr) Atomics.store(i, codePtr >> 2, Atomics.load(i, sb + vgiSab.TERMINAL_CODE));
        if (detailPtr) Atomics.store(i, detailPtr >> 2, Atomics.load(i, sb + vgiSab.TERMINAL_DETAIL));
        return 1;
    },
