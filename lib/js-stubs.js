addToLibrary({
    ...
    $vgiSab: {
        ...
        CTRL_BYTES: 64,
        i32: function () { return new Int32Array(Module.HEAPU8.buffer); },
        hdr: function () {
        ...
        ringWrite: function (ctl, writeLane, readLane, dataByte, ringCap, srcPtr, n) {
            var i = this.i32(); var u = Module.HEAPU8; var off = 0;
            while (off < n) {
        ...
        // Blocking SPSC ring read of up to n bytes into linear-memory dstPtr.
        ...
        ringRead: function (ctl, writeLane, readLane, closedLane, dataByte, ringCap, dstPtr, n) {
            var i = this.i32(); var u = Module.HEAPU8;
            for (;;) {
                var w = Atomics.load(i, ctl + writeLane);
                var r = Atomics.load(i, ctl + readLane);
                var avail = w - r;
                if (avail === 0) {
                    if (Atomics.load(i, ctl + closedLane) === 1) return 0;
                    Atomics.wait(i, ctl + writeLane, w, 250); continue;
                }
    ...
    // The bridge owns worker lifecycle; the data-path never calls this (ATTACH
    // does, via the main-thread proxy). Placeholder so the symbol resolves.
    vgi_wasm_ensure_worker__deps: ['$vgiSab'],
    vgi_wasm_ensure_worker: function (locationPtr) { return 0; },
    ...
    vgi_wasm_slot_open: function (locationPtr) {
        ...
        for (var s = 0; s < hdr.nSlots; s++) {
            ...
            if (Atomics.compareExchange(i, sb + vgiSab.STATE, 0, 1) === 0) {
                i[sb + vgiSab.C2W_W] = 0; i[sb + vgiSab.C2W_R] = 0; i[sb + vgiSab.C2W_CL] = 0;
                i[sb + vgiSab.W2C_W] = 0; i[sb + vgiSab.W2C_R] = 0; i[sb + vgiSab.W2C_CL] = 0;
                return s;
            }
        }
        return -1;
    ...
    vgi_wasm_slot_release: function (slot) {
        var i = vgiSab.i32(); var hdr = vgiSab.hdr();
        Atomics.store(i, (vgiSab.slotByte(hdr, slot) >> 2) + vgiSab.STATE, 0);
    },
});
