...
// --- main/client ---
// Two ABI-v1 regions can reuse slot zero in one SAB without crossing targets.
const regionBytes = ring.channelBytes(1, 64);
const multiSab = new SharedArrayBuffer(regionBytes * 2);
ring.initChannel(multiSab, 0, 1, 64);
ring.initChannel(multiSab, regionBytes, 1, 64);
const slotA = ring.slotOpen(multiSab, 0);
const slotB = ring.slotOpen(multiSab, regionBytes);
assert.strictEqual(slotA, 0);
assert.strictEqual(slotB, 0);
ring.stubWrite(multiSab, 0, slotA, Buffer.from([11]), 0, 1);
ring.stubWrite(multiSab, regionBytes, slotB, Buffer.from([22]), 0, 1);
const fromA = Buffer.alloc(1);
const fromB = Buffer.alloc(1);
assert.strictEqual(ring.workerRead(multiSab, 0, slotA, fromA, 0, 1), 1);
assert.strictEqual(ring.workerRead(multiSab, regionBytes, slotB, fromB, 0, 1), 1);
assert.strictEqual(fromA[0], 11);
assert.strictEqual(fromB[0], 22);

// Terminal errors are visible only to the claim that produced them.
const oldClaim = new Int32Array(multiSab)[ring.HEADER_BYTES >> 2];
ring.workerCloseError(multiSab, 0, slotA, 41, 9001);
assert.strictEqual(ring.stubReadWithTerminal(multiSab, 0, slotA, Buffer.alloc(1), 0, 1), ring.SAB_TERMINAL_ERROR);
assert.deepStrictEqual(ring.stubTerminalError(multiSab, 0, slotA), { code: 41, detail: 9001 });
ring.slotRelease(multiSab, 0, slotA);
assert.strictEqual(ring.slotOpen(multiSab, 0), slotA);
ring.workerCloseError(multiSab, 0, slotA, 99, 7, oldClaim);
assert.strictEqual(ring.stubTerminalError(multiSab, 0, slotA), null);
ring.slotRelease(multiSab, 0, slotA);
ring.slotRelease(multiSab, regionBytes, slotB);
